...
def parse_multistatus(content: str) -> list[vobject.base.Component]:
    ...
    for vcard_str in matches:
        try:
            vcard_str = vcard_str.replace("&lt;", "<").replace("&gt;", ">").replace("&amp;", "&")
...
def parse_bday(vcard, start_date: datetime.datetime, end_date: datetime.datetime) -> dict | None:
    """Parse a vCard and return a dictionary with event details if a birthday falls in the range.
    
    Returns None or dict with keys: start, end, summary, description, location
    """
    ...
    try:
        ...
        candidates = [
            candidate,
            datetime.date(current_year + 1, candidate.month, candidate.day) if candidate.month != 2 or candidate.day != 29 else datetime.date(current_year + 1, 3, 1)
        ]
        
        for d in candidates:
            d_dt = datetime.datetime.combine(d, datetime.time.min).replace(tzinfo=start_date.tzinfo)
        ...
        return {
            ...
            "summary": summary,
            "description": f"Happy {age}th Birthday!" if age > 0 and bday_val.year > 1900 else "Happy Birthday!",
            "location": ""
        }
